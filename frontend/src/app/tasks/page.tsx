...
export default function TasksPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 h-full bg-[var(--bg-primary)]">
      ...
      {/* Filters Bar */}
      <div className="px-6 py-3 border-b border-[var(--border-color)] flex gap-4 overflow-x-auto bg-black/5">
        <select className="bg-transparent border border-[var(--border-color)] text-[10px] uppercase font-mono tracking-widest px-2 py-1 rounded" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
          <option value="all">Tous les Statuts</option>
          <option value="running">En Cours</option>
          <option value="done">Terminé</option>
          <option value="failed">Échec</option>
        </select>
        <select className="bg-transparent border border-[var(--border-color)] text-[10px] uppercase font-mono tracking-widest px-2 py-1 rounded" value={approvalFilter} onChange={(e) => setApprovalFilter(e.target.value)}>
