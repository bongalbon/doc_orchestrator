...
def enqueue_task(
    ...
    if not effective_api_key and actor:
        storage = ProviderCredential.objects.filter(
            user=actor, provider=provider, is_active=True
        ).first()
        if storage:
            effective_api_key = storage.get_key()

    assigned_agent = choose_assigned_agent(
        prompt=prompt, requested_agent=requested_agent
    )
    ...
    AuditLog.objects.create(
        action="task_created", actor=actor, task=task, metadata={"queue_job_id": job.id}
    )
    return task
...
def running_snapshot() -> list[dict]:
    ...
    active_tasks = AgentTask.objects.filter(status="running").select_related(
        "assigned_agent"
    )
    active_workflows = Workflow.objects.filter(
        status__in=["thinking", "delegating", "reviewing"]
    ).select_related("manager_agent")
        ...
        snapshot.append(
            {
                "id": row.id,
                "type": "task",
                "agent_name": row.assigned_agent.name
...
def active_agents_snapshot() -> list[str]:
    ...
    running_tasks = AgentTask.objects.filter(status="running").select_related(
        "assigned_agent"
    )
    running_wf = Workflow.objects.filter(
        status__in=["thinking", "delegating", "reviewing"]
    ).select_related("manager_agent")
...
def start_workflow(
    ...
    AuditLog.objects.create(
        ...
        actor=user,
        metadata={"workflow_id": workflow.id, "job_id": job.id},
    )
    return workflow
